...
def clean_svi_data(dataframe, variables):
    ...
    for variable in variables:
        # replace -999 with 0
        dataframe[variable] = dataframe[variable].replace(-999, 0)

        # drop -999
        # dataframe = dataframe[dataframe[variable] != -999]
    return dataframe

def filter_svi_data_by_percentile(dataframe, variable, percentile):
    """Filter the SVI data based on a given percentile."""

    percentile_value = dataframe[variable].quantile(percentile)
    filtered_df = dataframe[dataframe[variable] < percentile_value]

    return filtered_df


def generate_adjacent_counties(dataframe,variable, percentile):
...
def plot_simplicial_complex(dataframe,V,variable,results_path):
    """Plot the simplicial complex."""

    # city centroids
    # sortedID used in persistence image code for the FIPS
    city_coordinates = {city.FIPS: np.array((city.geometry.centroid.x, city.geometry.centroid.y)) for _, city in dataframe.iterrows()}
    ...
    plt.savefig(f'{results_path}/simplicial_complex_{variable}.png')
    plt.close()
...
def generate_scaled_marginal_variance(simplices,data_frame, variable_name):
    ...
    filtered_counties_df = data_frame.loc[data_frame["sortedID"].isin(selected_counties)]
...
if __name__ == "__main__":
    ...
    # Download the SVI data for a required state - county level - ESRI Geodatabase (https://www.atsdr.cdc.gov/placeandhealth/svi/data_documentation_download.html)
    svi_gdb_path = "/Users/h6x/ORNL/git/ornl-scaled-marginal-variance/data/SVI2018_WYOMING_COUNTY.gdb"
    results_path = "/Users/h6x/ORNL/git/ornl-scaled-marginal-variance/results"
    ...
    for variable in tqdm(selected_variables, desc="Processing variables"):
        ...
        adjacencies_list, adjacent_counties_df, county_list = generate_adjacent_counties(df_one_variable, variable, percentile)
        ...
        # returns the simplicial complex connected compontn,edges and triangles (as a list of lists-which  cointains the sorted ID of the counties)
        ...
        # Plot the simplicial complex # plot will be saved in the results folder #change the path to save the plot in the desired location in the plot_simplicial_complex function
        ...
        df_one_variable.plot(column=variable + '_marginal_variance', cmap='OrRd', linewidth=0.5, ax=axes, edgecolor='black', legend=True) 
        df_one_variable.boundary.plot(ax=axes, color='black', linewidth=0.5)
        axes.set_title(variable + '_marginal_variance', fontsize=12)  # Adjust the fontsize here
        axes.set_axis_off()
